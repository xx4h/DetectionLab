...
echo "[$(date +%H:%M:%S)]: Prepare caching folder"
...
install_guacamole() {
  ...
  wget -P /vagrant/cache -nc --progress=bar:force "https://apache.org/dyn/closer.lua/guacamole/1.3.0/source/guacamole-server-1.3.0.tar.gz?action=download" -O guacamole-server-1.3.0.tar.gz
  tar -xf /vagrant/cache/guacamole-server-1.3.0.tar.gz && cd guacamole-server-1.3.0 || echo "[-] Unable to find the Guacamole folder."
